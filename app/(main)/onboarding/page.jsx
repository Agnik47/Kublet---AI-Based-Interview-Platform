'use client'
import PageHeader, { GoldTitle, GrayTitle, SectionLabel } from '@/components/reusables'
import { CATEGORIES, ONBOARDING_ROLES, ROLES, YEARS_OPTIONS } from '@/lib/data'
...
import { completeOnebording } from '@/actions/onboarding'

const OnboardingPage = () => {
  const router = useRouter();

  const {data , loading, fn: onboardingFn} = UseFetch(completeOnebording);
  ...
  return (
    <main className="min-h-screen bg-black px-6 py-0 flex flex-col items-center">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-serif text-5xl leading-tight tracking-tighter mt-1">
            ...
            <br />
            <GoldTitle>using Kublet?</GoldTitle>
          </h1>
        ...
        {!role && (
          <div className="grid grid-cols-2 gap-4 w-full cursor-pointer">
                ...
                <div className="flex flex-wrap gap-2">
                  {YEARS_OPTIONS.map((opt) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setForm((p) => ({ ...p, yearExp: opt.value }))
                      }
                      className={`text-xs px-4 py-2 rounded-lg border ${form.yearExp === opt.value
                  ...
                  {CATEGORIES.map((cat) => {
                    ...
                    return (
                      <button
                        ...
                        className={`text-xs px-4 py-2 rounded-lg border ${active
                            ? "border-amber-400/40 bg-amber-400/10 text-amber-400"
                            : "border-white/10 text-stone-500"
                          }`}
                      >
