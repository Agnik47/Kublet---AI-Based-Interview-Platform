...
/**
 * Send Booking Confirmation Emails to Interviewee and Interviewer
 */
export async function sendBookingEmails({ booking, interviewee, interviewer }) {
  ...
  try {
    await resend.emails.send({
      ...
      subject: `Confirmed: Mock Interview with ${interviewer.name}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
  ...
  // 2. Email to the Interviewer (Upcoming Interview Notification)
  try {
    await resend.emails.send({
      from: "Kublet <onboarding@resend.dev>",
      to: interviewer.email,
      subject: `New Booking: Mock Interview with ${interviewee.name}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="utf-8">
          <title>Upcoming Session Confirmed</title>
        </head>
        <body style="background-color: #000000; color: #e2e8f0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; margin: 0; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #0f0f11; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 40px; box-shadow: 0 4px 30px rgba(0, 0, 0, 0.5);">
            ...
            <!-- Main Heading -->
            <h2 style="font-size: 20px; font-weight: 600; color: #ffffff; text-align: center; margin-top: 0;">New Interview Booked!</h2>
            <p style="font-size: 14px; color: #94a3b8; text-align: center; line-height: 1.6; margin-bottom: 30px;">
              Hello ${interviewer.name}, an interviewee has booked a mock interview session with you. Here are the details:
            </p>
            ...
            <div style="background-color: #141417; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 24px; margin-bottom: 30px;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 8px 0; font-size: 12px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; width: 30%;">Candidate</td>
                  <td style="padding: 8px 0; font-size: 14px; color: #f1f5f9; font-weight: 500;">
                    <strong>${interviewee.name}</strong><br/>
                    <span style="font-size: 12px; color: #94a3b8;">${interviewee.email}</span>
                  </td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; font-size: 12px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Date & Time</td>
                ...
                <tr>
                  <td style="padding: 8px 0; font-size: 12px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Earnings</td>
                  <td style="padding: 8px 0; font-size: 14px; color: #fbbf24; font-weight: 600;">+ ${booking.creditsCharged} Credits ($${(booking.creditsCharged * 5).toFixed(2)})</td>
                </tr>
              </table>
            </div>
            ...
            <!-- Footer Notes -->
            <div style="border-top: 1px solid rgba(255,255,255,0.05); padding-top: 20px; font-size: 12px; color: #6b7280; text-align: center; line-height: 1.5;">
              Please make sure to join the call on time to guide the candidate. Visit your <a href="${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/dashboard" style="color: #fbbf24; text-decoration: none;">Interviewer Dashboard</a> to manage slots.<br/>
              Thank you for mentoring with Kublet!
            </div>
    ...
    console.log(`✅ Upcoming interview email sent successfully to interviewer: ${interviewer.email}`);
  } catch (err) {
    console.error("❌ Failed to send upcoming interview email to interviewer:", err);
  }
}
